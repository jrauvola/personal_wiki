...
- **autoresearch-hybrid validation run on topic "MobileCLIP2 R@10 measurements on Hexagon NPU"** (the topic that defeated both tools in the prior A/B test). Hybrid mode SUCCEEDED — surfaced 3 unique S2-citation-graph finds (AWQ, RegCache, TinyCLIP) and 1 unique WebSearch find (AXERA's MobileCLIP2 HuggingFace deployment page with measured latencies). Filed: synthesis [[questions/Research - MobileCLIP2 R@10 on Hexagon NPU]], 3 sources, 1 concept (activation-aware quant decision tree). Decisive read: AXERA's table confirms team's 13.7 ms latency = MobileCLIP2-S2 (MCi2), NOT S4. Routing rule validated in production. Skill performed as designed.
